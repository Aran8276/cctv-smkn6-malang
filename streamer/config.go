...
type StreamST struct {
	...
	RunLock      bool   `json:"-"`
	// Add the stop channel, ignored by JSON marshalling
...
func (element *ConfigST) RunIFNotRun(uuid string) {
	...
	if tmp, ok := element.Streams[uuid]; ok {
		// Make sure a stop channel exists if it's nil
		if tmp.stop == nil {
			tmp.stop = make(chan struct{})
		}
		if tmp.OnDemand && !tmp.RunLock {
			tmp.RunLock = true
			element.Streams[uuid] = tmp
			// Pass the stop channel to the worker loop
			go RTSPWorkerLoop(uuid, tmp.URL, tmp.OnDemand, tmp.DisableAudio, tmp.Debug, tmp.stop)
		}
	}
}
...
func loadConfig() *ConfigST {
	...
	if err == nil {
		...
		// This block runs if the config file doesn't exist
		// We can leave the flag parsing for that case.
	...
	// After attempting to load from file or flags, we check if the Streams map is nil.
	// If it is, we initialize it. This makes the program robust against an empty config file.
	if tmp.Streams == nil {
		tmp.Streams = make(map[string]StreamST)
	}

	// Now we can safely iterate and initialize the nested maps.
	// This loop will do nothing if the map is empty, but it won't panic.
